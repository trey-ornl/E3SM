...
#define TREY
    ...
    if ((m_rsplit > 0) && (!m_theta_hydrostatic_mode) && (m_theta_advection_form == AdvectionForm::NonConservative)) {
      ...
      Kokkos::parallel_for(
        "caar loop pre-boundary exchange lambda",
        TeamPolicy(m_num_elems, NPNP, WARP_SIZE).
        set_scratch_size(0, Kokkos::PerTeam(REAL_PER_NPNP + REAL_PER_THREAD + 2 * REAL_PER_POINT)),
        KOKKOS_LAMBDA(const Team &team) {
...
#pragma nounroll
              ...
              const Real v0 = v00[iz];
              const Real v1 = v01[iz];
...
//#pragma nounroll
              ...
              s0 *= sphere_rrearth;
              s1 *= sphere_rrearth;
              vt0 += dinv00 * s0 + dinv01 * s1;
              vt1 += dinv10 * s0 + dinv11 * s1;
      ...
      Kokkos::parallel_for(
        ...
        KOKKOS_LAMBDA(const Team &team) {
          ...
          if (iz == 0) w_np1[NUM_LEV] = w_nm1[NUM_LEV] * scale3_spheremp + w_tens[NUM_LEV] * dt_spheremp;
